...
ON CHOOSE OF BtnDone IN FRAME DEFAULT-FRAME /* Salvesta ja Välju */
DO:
  ...
  &ELSE
      ...
      MESSAGE 'Muudatuste salvestamine ebaõnnestus'.
  &ENDIF
END.
...
ON CHOOSE OF BtnExit IN FRAME DEFAULT-FRAME /* Välju */
DO:
  &IF "{&PROCEDURE-TYPE}" EQ "SmartPanel" &THEN
    &IF "{&ADM-VERSION}" EQ "ADM1.1" &THEN
      RUN dispatch IN THIS-PROCEDURE ('exit').
...
ON LEAVE OF setup.raamat_tahtaeg IN FRAME DEFAULT-FRAME /* Raamatu laenutamise perioodi pikkus päevades */
DO:
...
ON LEAVE OF setup.uusraamat_tahtaeg IN FRAME DEFAULT-FRAME /* 'Uue raamatu' laenutuse pikkus päevades */
DO:
...
ON LEAVE OF setup.uus_raamat IN FRAME DEFAULT-FRAME /* 'Uue raamatu' vanus päevades */
DO:
...
ON LEAVE OF setup.vaheraamat_tahtaeg IN FRAME DEFAULT-FRAME /* 'Vähese eksemplaride arvuga' raamatu laenutuse pikkus */
DO:
...
ON LEAVE OF setup.vahe_raamat IN FRAME DEFAULT-FRAME /* Eksemplaride arv, millest alates on laenutus lühem */
DO:
